...
int changeMakingRecursive(unsigned int C[], std::vector<unsigned int> Stock, int n, int T, std::vector<unsigned int> &usedCoins) {
    if(T < 0) {
        return INF;
    }
    ...
    std::vector<unsigned int> copy = usedCoins;
...
bool changeMakingBacktracking(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
    std::vector<unsigned int> ans(n,0);
    std::vector<unsigned int> st(Stock, Stock+n);

    int coins = changeMakingRecursive(C,st,n,T,ans);
    for(int i = 0; i < n; i++){
        usedCoins[i] = ans[i];
    }
    return coins != INF;
